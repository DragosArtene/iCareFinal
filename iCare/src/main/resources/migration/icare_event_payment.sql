...
CREATE TABLE `event_payment` (
  ...
  KEY `pay_id` (`pay_id`),
  CONSTRAINT `event_id_fk_pay` FOREIGN KEY (`event_id`) REFERENCES `event` (`event_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `pay_id_fk` FOREIGN KEY (`pay_id`) REFERENCES `payment` (`pay_id`) ON DELETE CASCADE ON UPDATE CASCADE
